...
void DFRobot_ESP_PH_WITH_ADC::begin(int EepromStartAddress) {
    ...
    this->_neutralVoltage = EEPROM.readFloat(
            this->_eepromStartAddress); //load the neutral (pH = 7.0)voltage of the pH board from the EEPROM
    ...
    this->_acidVoltage = EEPROM.readFloat(this->_eepromStartAddress +
                                          (int) sizeof(float)); //load the acid (pH = 4.0) voltage of the pH board from the EEPROM
...
float DFRobot_ESP_PH_WITH_ADC::readPH(float voltage, float temperature) {
    ...
    float slope = (7.0 - 4.0) / ((this->_neutralVoltage - PH_7_AT_25) / 3.0 - (this->_acidVoltage - PH_7_AT_25) /
    ...
    switch (mode) {
        ...
        case 2:
            if (enterCalibrationFlag) {
                // buffer solution:7.0
                // 7795 to 1250
                if ((this->_voltage > PH_VOLTAGE_NEUTRAL_LOW_LIMIT) &&
                    (this->_voltage < PH_VOLTAGE_NEUTRAL_HIGH_LIMIT)) {
                    Serial.println();
                    ...
                    //buffer solution:4.0
                    //1180 to 1700
                else if ((this->_voltage > PH_VOLTAGE_ACID_LOW_LIMIT) &&
                         (this->_voltage < PH_VOLTAGE_ACID_HIGH_LIMIT)) {
                    Serial.println();
