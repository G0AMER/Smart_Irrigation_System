...
DFRobot_ESP_EC::DFRobot_ESP_EC() {
    this->_ecvalue = 0.0;
    this->_kvalue = 9.994;
    this->_kvalueLow = 9.994;
    this->_kvalueHigh = 9.994;
...
void DFRobot_ESP_EC::begin(int EepromStartAddress) {
    ...
    if (this->_kvalueLow == float() || isnan(this->_kvalueLow)) {
        this->_kvalueLow = 9.994; // For new EEPROM, write default value( K = 1.0) to EEPROM
        EEPROM.writeFloat(this->_eepromStartAddress, this->_kvalueLow);
        EEPROM.commit();
    }

    this->_kvalueHigh = EEPROM.readFloat(
            this->_eepromStartAddress + (int) sizeof(float)); //read the calibrated K value from EEPROM
    if (this->_kvalueHigh == float() || isnan(this->_kvalueHigh)) {
        this->_kvalueHigh = 9.994; // For new EEPROM, write default value( K = 1.0) to EEPROM
...
void DFRobot_ESP_EC::ecCalibration(byte mode) {
    ...
    switch (mode) {
        ...
        case 1:
            ...
            Serial.println(
                    F(">>>Please put the probe into the 1413us/cm or 2.76ms/cm or 12.88ms/cm buffer solution<<<"));
            Serial.println(
                    F(">>>Only need two point for calibration one low (1413us/com) and one high(2.76ms/cm or 12.88ms/cm)<<<"));
            Serial.println();
            break;
        ...
        case 2:
            if (enterCalibrationFlag) {
                if ((this->_rawEC > RAWEC_1413_LOW) && (this->_rawEC < RAWEC_1413_HIGH)) {
                    Serial.print(
                            F(">>>Buffer 1.413ms/cm<<<"));                            //recognize 1.413us/cm buffer solution
                    ...
                    Serial.print(
                            F(">>>Buffer 2.76ms/cm<<<"));                            //recognize 2.76ms/cm buffer solution
                    ...
                    Serial.print(
                            F(">>>Buffer 12.88ms/cm<<<"));                            //recognize 12.88ms/cm buffer solution
